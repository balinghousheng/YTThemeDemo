...
//    NSArray<DKThemeVersion *> *themes = [DKColorTable sharedColorTable].themes;
    NSArray<DKThemeVersion *> *themes = [DKColorManager sharedColorTable].themes;
...
//    DKColorTable *colorTable = [DKColorTable sharedColorTable];
    DKColorManager *colorTable = [DKColorManager sharedColorTable];
...
+ (DKImagePicker)pickerWithImageName:(NSString *)imageName {
    return [self pickerWithImageName:imageName resizeable:NO];
}

+ (DKImagePicker)pickerWithImageName:(NSString *)imageName resizeable:(BOOL)resizeable {
    return ^(DKThemeVersion *themeVersion) {
        BOOL isVersionNormal = [[DKNightVersionManager sharedManager].themeVersion isEqualToString:DKThemeVersionNormal];
        NSString *pre = isVersionNormal ? @"Skin/Day" : @"Skin/Night";
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@", pre, imageName]];
        if (image) {
            if (resizeable) {
                image = [image stretchableImageWithLeftCapWidth:image.size.width topCapHeight:image.size.height];
            }
            return image;
        } else {
...
+ (DKImagePicker)pickerWithImageName:(NSString *)imageName edgeInsets:(UIEdgeInsets)edgeInsets {
    return ^(DKThemeVersion *themeVersion) {
        BOOL isVersionNormal = [[DKNightVersionManager sharedManager].themeVersion isEqualToString:DKThemeVersionNormal];
        NSString *pre = isVersionNormal ? @"Skin/Day" : @"Skin/Night";
        UIImage *image = [UIImage imageNamed:[NSString stringWithFormat:@"%@/%@", pre, imageName]];
